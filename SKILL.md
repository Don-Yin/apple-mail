...
production must not drive visible Apple Mail compose windows, draft deletion, message deletion, sending, forwarding, replies, or moves. those paths have crashed Mail during testing, including crashes in Mail's scripting delete handlers.
...
| `delete-email --message-ids MID [MID...]` or `--ids ID [ID...]`             | dev-only Mail.app deletion; disabled by default | ~1-3 s    |
...
3. propose an action to the user. do not run live `move-email` or `delete-email` in production; those commands are disabled unless the per-command development override is set.
...
4. present the confirmed list to the user. live `batch-move` and `delete-email` are disabled in production; use provider-side tools/APIs for bulk actions instead of Mail.app scripting.
...
1. production write/delete safety -- the only production Mail write is hidden `compose-draft` create/save. do not use visible compose windows or live Mail.app amend/send/delete/move/reply/forward commands in production.
2. always draft first -- never send without creating and confirming a draft
3. never delete without confirmation -- always confirm with the user before deletion, and use provider-side tooling rather than Mail.app scripting
...
3. for development-only live Mail writes/deletes, use the old Mail-side verification checks only on a disposable mailbox with `--allow-live-mail-mutation`; do not run them against the user's production mailbox.
